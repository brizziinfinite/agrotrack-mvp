  ...
  async function handleSaveFence() {
    ...
    try {
      ...
      if (result.success) {
        ...
        setSaveError(result.error || 'Não foi possível salvar a cerca.')
      }
    } catch (err: unknown) {
      const message = err instanceof Error ? err.message : 'Falha ao salvar cerca virtual'
  ...
  const baseLayers = useMemo(
    () => [
      ...
      },
      {
        ...
        attribution:
          'Tiles &copy; Esri &mdash; Source: Esri, USGS, NOAA, EPA, and the GIS User Community'
      },
      {
        id: 'satellite',
        name: 'Satélite',
    ...
    <div className={cn("relative w-full rounded-2xl overflow-hidden border shadow-md bg-slate-900/50", className || "h-[520px]")}>
      ...
      >
        <TileLayer
          ...
          subdomains={selectedBase.subdomains as string[] | undefined}
        />
      ...
      {/* Visualizações ao estilo Traccar */}
      <div className="pointer-events-none absolute right-4 top-4 z-[999] flex max-w-xl flex-col gap-2 items-end">
        <button
          type="button"
          aria-label="Alternar visualização do mapa"
          aria-expanded={showLayerMenu}
          onClick={() => setShowLayerMenu((v) => !v)}
          className="pointer-events-auto flex h-12 w-12 items-center justify-center rounded-full border border-white/30 bg-slate-900/80 text-white shadow-lg shadow-black/30 backdrop-blur transition hover:-translate-y-[1px] hover:shadow-xl focus:outline-none focus:ring-2 focus:ring-emerald-400"
        >
            ...
            <div className="flex flex-col divide-y divide-slate-100">
              {baseLayers.map((layer) => (
                <button
                  ...
                  className={`flex w-full items-center gap-2 px-4 py-3 text-left transition ${selectedBaseId === layer.id
                    ? 'bg-emerald-50/80 text-emerald-900'
                    : 'bg-white text-slate-800 hover:bg-slate-50'
                  }`}
                >
                  <span
                    className={`h-3 w-3 rounded-full border ${selectedBaseId === layer.id ? 'border-emerald-600 bg-emerald-500' : 'border-slate-300 bg-white'}`}
                    aria-hidden
                  />
      ...
      <div className="pointer-events-none absolute right-4 top-4 z-[999] w-80 space-y-3">
        <div className="rounded-xl border border-white/40 bg-white/90 p-4 shadow-lg shadow-black/20 backdrop-blur">
          ...
          <p className="mt-2 text-xs text-emerald-700 font-medium">
            Cercas ativas: {virtualFences.length}
          </p>
        </div>
        ...
        {virtualFences.length > 0 && (
          <div className="max-h-48 overflow-y-auto rounded-xl border border-white/40 bg-white/90 p-3 shadow-lg shadow-black/20 backdrop-blur">
            {virtualFences.map((fence, index) => (
              <div key={fence.id} className="flex items-center justify-between gap-3 rounded-lg bg-emerald-50/70 px-3 py-2 mb-2 last:mb-0">
                <div>
                  <p className="text-xs font-semibold text-emerald-800">
                    Cerca #{index + 1} · {fence.type === 'polyline' ? 'Lápis' : fence.type === 'circle' ? 'Círculo' : 'Polígono'}
                  </p>
                  <p className="text-[11px] text-emerald-700">
                    Pontos: {fence.coordinates.length}{fence.radius ? ` · Raio: ${Math.round(fence.radius)}m` : ''}
                  </p>
                </div>
                <button
                  type="button"
                  onClick={() => setActiveFenceId(fence.id)}
                  className={`pointer-events-auto h-6 w-6 rounded-full border ${activeFenceId === fence.id ? 'bg-emerald-600 border-emerald-600' : 'bg-white border-emerald-400'}`}
        ...
        <div className="pointer-events-auto rounded-xl border border-white/40 bg-white/95 p-4 shadow-lg shadow-black/20 backdrop-blur space-y-3">
          <div>
